// src/components/FactStatusBar.tsx
// Daily fact answer grid. Same UX as TriviaStatusBar but for facts.
// Blue = answered, grey = skipped, purple pulse = catch-up gap.
...
interface FactStatusBarProps { userId: string; }
interface GridCell { result: boolean | null; }
interface SeasonMarker { column: number; label: string; }
interface GridRow { userId: string; cells: GridCell[]; answered: number; seasonGaps: number; }
interface FactGridData { rows: GridRow[]; totalColumns: number; seasonMarkers: SeasonMarker[]; seasonStartCol: number; todayColumn: number; }

const CELL = 13;
const GAP = 3;
const NAME_W = 56;

const FactStatusBar: React.FC<FactStatusBarProps> = ({ userId }) => {
  const theme = useTheme();
  const scrollRef = useRef<HTMLDivElement>(null);

  const [grid, setGrid] = useState<FactGridData | null>(null);
  useEffect(() => {
    let cancelled = false;
    apiService.request<FactGridData>(
      `/fact-grid?userId=${encodeURIComponent(userId)}`,
      { method: 'GET' },
    ).then(data => { if (!cancelled) setGrid(data); }).catch(() => {});
    return () => { cancelled = true; };
  }, [userId]);

  // Scroll to right on load AND when tab becomes visible
  useEffect(() => {
    if (!grid || !scrollRef.current) return;
  ...
  return (
    <Box sx={{ mb: 1.5 }}>
              ...
              <React.Fragment key={row.userId}>
                ...
                <Box sx={{
                  ...
                  <Box sx={{
                    ...
                    pr: 0.75, bgcolor: theme.palette.background.default,
                  }}>
                    <Typography sx={{
                    ...
                    return (
                      <React.Fragment key={ci}>
                        ...
                        <Box sx={{
                          ...
                          // Blue = answered, grey = skipped, purple pulse = catch-up
      ...
      <Box sx={{ display: 'flex', gap: 1.5, mt: 0.75, ml: `${NAME_W}px`, flexWrap: 'wrap' }}>
        {[
          { color: '#2196f3', label: 'Answered' },
          { color: alpha(theme.palette.text.disabled, 0.25), label: 'Skipped' },
